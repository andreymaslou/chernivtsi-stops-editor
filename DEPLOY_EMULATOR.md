...
- **Защиты сейчас нет — и это намеренно.** Стенд открыт наружу (в `docker-compose.yml` стоит `"8000:8000"`), чтобы проверять эмулятор с телефона без nginx. Данные не секретны (остановки/расписания и так открыты), поэтому «закрывать» нечего; шторка-пароль (`btoa('15201520')`) из `web/editor.html` убрана — она не защищала (видна в исходнике), но блокировала отрисовку страницы в UI-тестах. Реальная защита нужна только при передаче стенда в боевое приложение — `auth_basic` в nginx (см. выше).
