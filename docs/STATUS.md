...
Ожидаемое: `10 passed`; UI-проверка печатает 24 `OK` и `CHECK_EMULATOR_OK`
(на 2026-09-17: 60–70 маркеров, у всех ненулевой курс, 61 из 61 сменили позицию в
...
плана («сідати: 5-032» → `target` у 5-032), легенда карти: 4 ряда за
спецификацией, клик и свайп по ней карту не двигают, 129 px из 390 px на
мобильном, консоль чистая, скриншоты в `tools/ui/out/`); паритет JSON
...
6. З брифу `docs/BRIEF-emulator-vehicles-visual.md` ще без спеки: формат попапа
   машини (питання 8), трейл/виділення обраної машини (питання 7), кластеризація
   на zoom < 14 (питання 9).
