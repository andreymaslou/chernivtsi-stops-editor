...
7. **Напрямок ТС у підборі «першого потрібного ТС»** — бриф
   `docs/BRIEF-router-direction.md` (жёсткий фильтр: да; «розумний розворот»: ні,
   бо симулятор не розвертає машини, а реальний трекер не віддає `direction`).
   У дереві вже лежить незакомічений чернетка Шага 1 у `router_layer.py` — її
   треба переписати за брифом (`_FLEET_KEY_FIELDS` + нормалізація напрямку +
   тести), а посторонні правки (`sim_layer.py` борт +500, `web/admin.js`
   Google Maps, `web/emulator.html` адаптивна верстка,
   `tools/apply_on_demand_slang.py`) закоммитити окремо.
   Замер: `python tools/perf/probe_direction.py --rev 916c286` — зустрічних
   кандидатів 1149 → 0, планів 9/30, ніг 11; на парку без поля `direction`
   (реальний трекер) лишається 1149 → потрібен Шаг 3 (курс) з брифа.
